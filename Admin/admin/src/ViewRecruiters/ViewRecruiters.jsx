...
const ViewRecruiters = () => {
   ...
   const url="http://localhost:5000/admin/all-recruiters"
  ...
  try {
    ...
    if (response.data.success) {
      alert("Recruiters Fetched Successfully");
      setRecruiter(response.data.allRecruiters);
      console.log(response.data.allRecruiters);
    } else {
      alert("Error");
    }
  } catch (error) {
    ...
    const deleteRecruiter = async(id)=>{
      const url=`http://localhost:5000/admin/delete-recruiter/${id}`;
      try{
